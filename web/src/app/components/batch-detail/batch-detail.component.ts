...
onDeleteClick(contact: Contact) {
  ...
  if (batchId === null) {
    // Handle the null case here. For example, show an error message.
    console.error('Batch ID is null');
    return;
  }

  if (window.confirm('Are you sure you want to remove this contact from the batch?')) {
    this.contactService.removeBatchFromContact(contact.id, batchId).subscribe(() => {
